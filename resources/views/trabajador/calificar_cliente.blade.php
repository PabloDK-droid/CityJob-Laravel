...
@section('content')
<style>
    :root { --cyan:#00C3FF; --cyan-dim:#0094cc; --navy:#00152B; --navy-mid:#002647; --text-muted:#8BAAC8; --border:rgba(0,195,255,0.15); --white:#FFFFFF; }
    ...
    .cj-main { flex:1; padding:2.5rem; overflow-y:auto; display:flex; justify-content:center; }
    ...
    .page-sub { color:var(--text-muted); font-size:.9rem; margin-bottom:1.5rem; }
    ...
    .star-rating { display:flex; gap:.75rem; justify-content:center; font-size:2.5rem; }
    ...
    textarea { width:100%; padding:.85rem 1rem; min-height:120px; background:rgba(255,255,255,.06); border:1px solid var(--border); border-radius:.75rem; color:var(--white); font-family:'Instrument Sans',sans-serif; font-size:.92rem; resize:vertical; outline:none; transition:border-color .2s; }
    textarea:focus { border-color:var(--cyan); box-shadow:0 0 0 3px rgba(0,195,255,.1); }
    textarea::placeholder { color:rgba(139,170,200,.35); }
    .btn-submit { width:100%; padding:.9rem; background:var(--cyan); color:var(--navy); font-family:'Syne',sans-serif; font-weight:700; font-size:1rem; border:none; border-radius:.75rem; cursor:pointer; transition:all .25s; }
    .btn-submit:hover { background:var(--cyan-dim); transform:translateY(-2px); }
    .btn-submit:disabled { background:rgba(0,195,255,.2); color:rgba(255,255,255,.3); cursor:not-allowed; transform:none; }
...
<div class="cj-page">
    ...
    <main class="cj-main">
        <div class="rating-wrap">
            <a href="{{ route('trabajador.misCalificaciones') }}" class="btn-back">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="15 18 9 12 15 6"/></svg>
                Volver a mis calificaciones
            </a>
            <h1 class="page-title">Calificar Cliente</h1>
            <p class="page-sub">Comparte tu experiencia con este cliente</p>
            ...
            <form action="{{ route('trabajador.guardarCalificacionCliente') }}" method="POST">
                @csrf
                <input type="hidden" name="id_contratacion" value="{{ $contratacion->id_contratacion }}">
                <input type="hidden" name="id_cliente" value="{{ $contratacion->id_cliente }}">
                <input type="hidden" name="calificacion" id="calificacion-input" value="0">

                <div class="stars-section">
                    <p class="stars-label">¿Cómo fue trabajar con este cliente?</p>
                ...
                <div class="form-group">
                    ...
                    <textarea name="comentario" placeholder="Describe tu experiencia con este cliente..."></textarea>
                </div>

                <button type="submit" class="btn-submit" id="submit-btn" disabled>Enviar Calificación</button>
            </form>
        </div>
    </main>
</div>
...
<script>
    ...
    function highlight(n) {
        stars.forEach(s => parseInt(s.dataset.rating) <= n ? s.classList.add('active') : s.classList.remove('active'));
    }
    stars.forEach(s => {
