...
<div>
    <h1>Historial Global de Contrataciones</h1>

    @if($rol === 'admin')
        <a href="{{ route('admin.dashboard') }}">← Volver al panel</a>
    @else
        <a href="{{ route('ingeniero.dashboard') }}">← Volver al panel</a>
    @endif
            ...
            <tbody>
                @foreach($contrataciones as $contratacion)
                    <tr>
                        ...
                        <td>
                            @if($rol === 'admin')
                                <a href="{{ route('admin.historial.detalle', $contratacion->id_contratacion) }}"
                                   style="background:#0066ff;color:white;padding:6px 12px;border-radius:4px;text-decoration:none;font-size:13px;">
                                    Ver Detalle
                                </a>
                            @else
                                <a href="{{ route('ingeniero.historial.detalle', $contratacion->id_contratacion) }}"
                                   style="background:#0066ff;color:white;padding:6px 12px;border-radius:4px;text-decoration:none;font-size:13px;">
                                    Ver Detalle
                                </a>
                            @endif
