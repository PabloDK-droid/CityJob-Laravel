...
<style>
    ...
    .inline-input { padding:.38rem .7rem; background:rgba(255,255,255,.06); border:1px solid var(--border); border-radius:.5rem; color:var(--white); font-family:'Instrument Sans',sans-serif; font-size:.82rem; outline:none; transition:border-color .2s; }
    ...
    .form-group input { padding:.7rem 1rem; background:rgba(255,255,255,.06); border:1px solid var(--border); border-radius:.65rem; color:var(--white); font-family:'Instrument Sans',sans-serif; font-size:.9rem; outline:none; transition:border-color .2s; width:100%; }
    ...
    <main class="cj-main">
        ...
        @if($servicios->count() > 0)
            <div class="table-wrap">
                <table>
                    <thead><tr><th>#</th><th>Nombre del servicio</th><th>Precio base</th><th>Contrataciones</th><th>Acciones</th></tr></thead>
                    <tbody>
                                ...
                                <td>
                                    <div class="td-actions">
                                        <form action="{{ route('admin.actualizarServicio', $s->id_servicio) }}" method="POST" style="display:contents">
                                            ...
                                            <input type="number" class="inline-input" name="precio_base" value="{{ $s->precio_base }}" required step="0.01">
                                            <button type="submit" class="btn-sm btn-update">Actualizar</button>
                                        </form>
                                        <form action="{{ route('admin.eliminarServicio', $s->id_servicio) }}" method="POST" style="display:contents">
                                            @csrf
                                            <button type="submit" class="btn-sm btn-delete" onclick="return confirm('¿Eliminar?')">Eliminar</button>
                                        </form>
                                    </div>
                                </td>
                            </tr>
        ...
        <div class="form-card">
            <form action="{{ route('admin.crearServicio') }}" method="POST">
                @csrf
                <div class="form-group"><label>Nombre del servicio</label><input type="text" name="nombre_servicio" required placeholder="Ej: Plomería"></div>
                <div class="form-group"><label>Precio base (MXN)</label><input type="number" name="precio_base" required step="0.01" placeholder="Ej: 500.00"></div>
